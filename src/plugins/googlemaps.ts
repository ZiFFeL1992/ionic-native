...
export class GoogleMapsBaseClass {
  constructor(private _oI: any) { }

  addEventListener(event: string): Observable<any> {
    return new Observable(
      (observer) => {
        this._oI.addEventListener(event, observer.next.bind(observer));
        return () => this._oI.removeEventListener(event, observer.next.bind(observer));
      }
    );
  }

  addListenerOnce(): Promise<any> {
    return new Promise<any>(
      resolve => this._oI.addListenerOnce(event, resolve)
    );
  }


  removeEventListener(): Observable<any> {
    return new Observable(
      (observer) => {
        return () => this._oI.removeEventListener(event, observer.next.bind(observer));
      }
  ...
  on(event: string): Observable<any> {
    return new Observable(
      (observer) => {
        this._oI.on(event, observer.next.bind(observer));
        return () => this._oI.off(event);
      }
    );
  }
   ...
   * @return {Promise<any>}
   */
  one(event: string): Promise<any> {
    return new Promise<any>(
      resolve => this._oI.one(event, resolve)
    );
  }
 ...
 * @private
 */
export class GoogleMapsMarker extends GoogleMapsBaseClass {
 ...
 * @private
 */
export class GoogleMapsCircle extends GoogleMapsBaseClass {
 ...
 * @private
 */
export class GoogleMapsPolyline extends GoogleMapsBaseClass {
  constructor(private _objectInstance: any) {
    super(_objectInstance);
  }
 ...
 * @private
 */
export class GoogleMapsPolygon extends GoogleMapsBaseClass {
 ...
 * @private
 */
export class GoogleMapsTileOverlay extends GoogleMapsBaseClass {
 ...
 * @private
 */
export class GoogleMapsGroundOverlay extends GoogleMapsBaseClass {
 ...
 * @private
 */
export class GoogleMapsKmlOverlay extends GoogleMapsBaseClass {
